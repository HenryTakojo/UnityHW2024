...
namespace EnhancedUI.EnhancedScroller
{
    ...
    public class EnhancedScroller : MonoBehaviour
    {
        ...
        public ScrollRectDragState ScrollRectDragState
        {
            get
            {
                return _scrollRectDragState;
            }
        }
        ...
        public int GetCellViewIndexAtPosition(float position)
        {
            ...
            for (int i = 0; i < _cellViewOffsetArray.Count; i++)
            {
                float Dis = Mathf.Abs((_cellViewOffsetArray[i] - _cellViewSizeArray[i] + (scrollDirection == ScrollDirectionEnum.Vertical ? padding.top : padding.left)) - (position));
        ...
        private ScrollRectDragState _scrollRectDragState;
        ...
        private void _ResetVisibleCellViews()
        {
            ...
            _delegate.GetCellViewRefreshIndex(startIndex, endIndex);
            ...
            while (i < _activeCellViews.Count)
            {
                ...
                {
                    var recycleCell = _activeCellViews[i];
                    recycleCell.active = false;
                    _delegate.GetCellViewVisibility(recycleCell);
        ...
        private void _RecycleAllCells()
        {
            ...
            {
                var recycleCell = _activeCellViews[0];
                recycleCell.active = false;
                _delegate.GetCellViewVisibility(recycleCell);
        ...
        private void _AddCellView(int cellIndex, ListPositionEnum listPosition)
        {
            ...
            _delegate.GetCellViewVisibility(cellView);
        ...
        private void _ScrollRect_OnValueChanged(Vector2 val)
        {
            ...
            if(maxVelocity > 0)
            {
                _scrollRect.velocity = new Vector2(
                    Mathf.Clamp(_scrollRect.velocity.x, -maxVelocity, maxVelocity),
                    Mathf.Clamp(_scrollRect.velocity.y, -maxVelocity, maxVelocity));
            }
            ...
            if (snapping && !_snapJumping)
            {
                ...
                if (Mathf.Abs(LinearVelocity) <= snapVelocityThreshold && LinearVelocity != 0)
                {
                    ...
                    if (!IsDrag && (loop || (!loop && normalized > 0 && normalized < 1.0f)))
                    {
